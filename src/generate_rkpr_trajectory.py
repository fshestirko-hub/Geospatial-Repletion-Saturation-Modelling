...
def generate_rkpr_trajectory():
    ...
    gif_path = os.path.join(plots_dir, "walk_rkpr_trajectory_2d.gif")
    png_path = os.path.join(plots_dir, "walk_rkpr_trajectory_2d.png")
    ...
    if os.path.exists(gif_path) and os.path.exists(png_path):
        print("DEBUG: RKPR trajectory plots already exist. Skipping generation.")
        return
    ...
    if active_session is not None:
        ...
        print("DEBUG: Reusing active Spark Session for RKPR Trajectory.")
    else:
        print("DEBUG: Initialising new Spark Session for RKPR Trajectory...")
        spark_builder = SparkSession.builder \
            .appName("HHAR-RKPR-Trajectory") \
    ...
    try:
        ...
        ax_traj.set_title("Reconstructed walk trajectory (Relative kinematic path reconstruction)", color='white', fontsize=12, pad=15)
        ...
        ax_gyro.set_title("Heading angle and rotational velocity", color='white', fontsize=11)
        ...
        # Save static PNG.
        print(f"DEBUG: Saving static RKPR Trajectory image to: {png_path}...")
        plt.savefig(png_path, dpi=150, facecolor=fig.get_facecolor(), edgecolor='none')
        print("DEBUG: Static RKPR image saved successfully.")
        ...
        print(f"DEBUG: Rendering RKPR Trajectory animation to: {gif_path}...")
        ani = animation.FuncAnimation(fig, update_anim, frames=len(frame_indices), interval=50, blit=False)
        ani.save(gif_path, writer='pillow', fps=20)
        print("DEBUG: Dynamic RKPR animation rendering complete!")
