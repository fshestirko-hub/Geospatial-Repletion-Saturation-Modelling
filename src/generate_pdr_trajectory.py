...
def generate_pdr_trajectory():
    ...
    print("DEBUG: Initializing Spark Session for PDR Trajectory...")
    ...
    try:
        ...
        df_acc_raw = spark.read.csv(acc_path, header=True, inferSchema=True)
        df_gyro_raw = spark.read.csv(gyro_path, header=True, inferSchema=True)
        ...
        # We target a single device nexus4_1 to keep streams perfectly synchronized
        ...
        # We step 0.70 meters (standard step length) in the direction of current heading.
        step_len = 0.70
        ...
        # ----------------------------------------------------
        # Matplotlib Premium Visualization (Dual Panel)
        # ----------------------------------------------------
        ...
        ax_traj.set_xlabel("X Position (meters)", color='#aaaaaa')
        ax_traj.set_ylabel("Y Position (meters)", color='#aaaaaa')
